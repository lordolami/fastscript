...
function signPayload(payload, secret, algo = "sha256") {
  const mac = createHmac(algo, secret).update(payload).digest("hex");
  return `${algo}=${mac}`;
}

function verifySignature(payload, header, secret, algo = "sha256") {
...
function isReplay(tsSeconds, maxSkewSec = 300) {
...
function replayStore({ dir = ".fastscript", ttlSec = 600 } = {}) {
  ...
  return {
    has(id) {
      const now = Date.now();
      for (const [key, exp] of Object.entries(state.seen)) {
        if (exp < now) delete state.seen[key];
      }
      persist();
...
export function verifyWebhookPayload(payload, headers, { secret, signatureHeader = "x-signature", timestampHeader = "x-timestamp", idHeader = "x-event-id", maxSkewSec = 300, replayDir = ".fastscript" } = {}) {
  const raw = Buffer.from(JSON.stringify(payload ?? {}));
  const sig = readHeader(headers, signatureHeader);
  const ts = readHeader(headers, timestampHeader);
  const eventId = readHeader(headers, idHeader);
